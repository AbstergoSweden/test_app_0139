...
export const GalleryItem: React.FC<GalleryItemProps> = ({ item }) => {
  ...
  return (
    <div className="relative aspect-square bg-slate-800 rounded-xl overflow-hidden group shadow-lg border border-slate-700/50 hover:border-orange-500 transition-all transform hover:scale-[1.02]">
      ...
      {/* Overlay */}
      <div className={`absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-3 z-20 ${!isLoaded ? 'hidden' : ''}`}>
      ...
      <div className={`absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity z-20 ${!isLoaded ? 'hidden' : ''}`}>
